...
class ConvLSTMCell(nn.Module):
    """
    Convolutional LSTM Cell: Used in ConvLSTM as individual "cells" to capture temporal dependencies.
    """
...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_size: Tuple[int, int], input_dim: int, hidden_dim: List[int], kernel_size: Tuple[int, int],
                 num_layers: int, batch_first: bool = False, bias: bool = True, return_all_layers: bool = False):
        """
        :param input_size:
        :param input_dim:
        :param hidden_dim:
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        convlstm = ConvLSTM(input_size=(4, 8), input_dim=512, hidden_dim=512, kernel_size=(3, 3), num_layers=2,
                            batch_first=True).to(device)
        # b t c h w
        fake_series = torch.zeros((5, 3, 512, 4, 8)).to(device)
        ...
        print("Output {} ".format(last_state[0][0].size()))
