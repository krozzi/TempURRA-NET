import torch
from tempurranet.model.old.unet import *
from tempurranet.model.old.lstm import *
...
class TempURRA(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.up1(output, decoder_batched_sequences[0][-1])
        x = self.up2(x, decoder_batched_sequences[1][-1])
        x = self.up3(x, decoder_batched_sequences[2][-1])
        x = self.up4(x, decoder_batched_sequences[3][-1])
        x = self.outc(x)
        # encoder
